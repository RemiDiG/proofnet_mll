(* Sequentialisation - A terminal ⅋ is sequentializing *)
...
From Yalla Require Export mll_prelim graph_more upath mll_def mll_basic mll_seq_to_pn
  mll_pn_to_seq_def.
...
Section Rem_node.
Context {G : proof_structure} {v : G} (V : vlabel v = ⊗ \/ vlabel v = ⅋) (T : terminal v).
...
(** Base graph for removing a node and its conclusion *)
Definition rem_node_graph_1 := induced ([set: G] :\ v :\ cv).

(* Then add new conclusions *)
Lemma lv_inside : lv \in setT :\ v :\ cv.
Proof.
  rewrite !in_set !in_set1 andb_true_r. splitb; apply/eqP => F.
...
Lemma v_neq_cv : v <> cv.
Proof. move=> F. have := vlabel_cv. rewrite -F. by destruct V as [V' | V']; rewrite V'. Qed.
...
Proof.
  ...
  - apply no_source_c, vlabel_cv.
  - move=> ?. contradict Hc. by apply ccl_eq.
  - move=> ?. contradict Hc. apply one_target_c; trivial. apply vlabel_cv.
  - move=> ?. contradict Hr. by apply right_eq2.
Qed.
...
Proof.
  move=> ?.
  rewrite /rem_node_transport /rem_node_transport'.
  case: {-}_ /boolP => In; cbnb. case_if.
  move: In. rewrite rem_node_removed !in_set !in_set1. introb.
Qed.

Lemma rem_node_transportK' : cancel rem_node_transport' rem_node_transport.
Proof.
  rewrite /rem_node_transport /rem_node_transport'.
  move=> [[[[[e E] | []] | ] | []] | ];
  case: {-}_ /boolP => In.
  - cbnb.
  - contradict E. by apply/negP.
  - contradict In. apply/negP. by rewrite rem_node_removed !in_set !in_set1 eq_refl andb_false_r.
...
Proof.
  ...
  move: Lin. rewrite rem_node_transport_edges => /imsetP[al Al ?]. subst el.
  move: Rin. rewrite rem_node_transport_edges => /imsetP[ar Ar ?]. subst er.
  move: LR. rewrite !flabel_rem_node_transport' => LR.
...
Proof.
  ...
  move: Lin. rewrite rem_node_transport_edges => /imsetP[al Al ?]. subst el.
  move: Rin. rewrite rem_node_transport_edges => /imsetP[ar Ar ?]. subst er.
  move: Cin. rewrite rem_node_transport_edges => /imsetP[ac Ac ?]. subst ec.
  move: Elrc. rewrite !flabel_rem_node_transport' => Elrc.
  rewrite !rem_node_transportK'. splitb.
  - move: Ll. destruct al as [[[[[? ?] | []] | ] | []] | ]; cbnb.
  - move: Rl. destruct ar as [[[[[? ?] | []] | ] | []] | ]; cbnb.
...
Proof.
  ...
  - move=> ?. apply map_f.
    ...
    + move: E. rewrite rem_node_removed !in_set !in_set1. introb.
    + by apply p_order_full.
  - move=> /mapP[a A Ha].
    assert (a = e).
    { move: Ha. unfold rem_node_transport. case: {-}_ /boolP => [In | /negP //].
      case: {-}_ /boolP => [In' | /negP-? //]; last by case_if.
      move=> /eqP. by cbnb => /eqP-->. }
    subst a.
    move: A. rewrite mem_filter. introb.
    by apply p_order_full.
Qed.
...
Proof.
  ...
  - apply/mapP; move=> [a A] /eqP.
    ...
    move: A. rewrite mem_filter => /andP[_ A].
  ...
  - apply/mapP; move=> [a A] /eqP.
    ...
    move: A. rewrite mem_filter => /andP[/eqP-A0 A].
    move: Ain. rewrite rem_node_removed // !in_set !in_set1. introb.
    apply p_order_full in A.
    contradict A.
